...
아래 계산 결과에서 자미두수 명반 데이터를 찾아 심층 해석을 ${lang !== 'ko' ? lang + '로' : '한국어로'} 작성하세요.
다음 항목을 각각 ${DEPTH_SENTENCES[depth]} 설명하세요:
1. 명궁 주성과 기본 운명 구조
2. 재백궁·관록궁 분석 (재물·직업)
3. 부처궁 분석 (연애·결혼)
...
출력 형식: 항목 번호와 소제목 포함, 자연스러운 문단.`,
  ...
  natal: (lang, depth) => `
당신은 서양 점성술 전문가입니다.
아래 계산 결과에서 네이탈 차트 데이터를 찾아 심층 해석을 ${lang !== 'ko' ? lang + '로' : '한국어로'} 작성하세요.
다음 항목을 각각 ${DEPTH_SENTENCES[depth]} 설명하세요:
1. 태양·달·상승궁 종합 (핵심 자아)
2. 주요 행성 배치와 하우스 의미
3. 주요 어스펙트 (각도) 해석
...
아래 계산 결과를 바탕으로 이 사람의 연애·결혼 운을 ${lang !== 'ko' ? lang + '로' : '한국어로'} 통합 해석하세요.
다음 항목을 각각 ${DEPTH_SENTENCES[depth]} 설명하세요:
...
출력 형식: 항목 번호와 소제목 포함.`,
  ...
  career: (lang, depth) => `
당신은 사주명리·자미두수·서양 점성술 전문가입니다.
아래 계산 결과를 바탕으로 이 사람의 직업·재물 운을 ${lang !== 'ko' ? lang + '로' : '한국어로'} 통합 해석하세요.
다음 항목을 각각 ${DEPTH_SENTENCES[depth]} 설명하세요:
...
출력 형식: 항목 번호와 소제목 포함.`,
  ...
  year: (lang, depth) => `
당신은 사주명리·자미두수·서양 점성술 전문가입니다.
아래 계산 결과를 바탕으로 2026년 올해 운세를 ${lang !== 'ko' ? lang + '로' : '한국어로'} 통합 분석하세요.
다음 항목을 각각 ${DEPTH_SENTENCES[depth]} 설명하세요:
1. 사주 2026 세운 분석
2. 자미두수 2026 유년 분석
3. 점성술 2026 트랜짓 핵심
...
아래 계산 결과를 바탕으로 세 시스템의 공통 메시지와 차이점을 찾아 인생 전체 흐름을 ${lang !== 'ko' ? lang + '로' : '한국어로'} 종합 해석하세요.
다음 항목을 각각 ${DEPTH_SENTENCES[depth]} 설명하세요:
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = promptFn(language || 'ko', depth || 'standard');
