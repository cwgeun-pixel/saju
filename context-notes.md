...
회원가입·결제 없이 전면 무료로 운영하기로 해서 유료 흐름을 차단했다. **파일은 지우지 않고 링크만 끊어** 유료화할 때 되살리기 쉽게 했다.

### 되살릴 때 만질 곳 (유료화 시 체크리스트)
1. `index.html`의 `// initAuthBtn();` 주석 해제 — 로그인/회원가입 버튼이 다시 나온다.
2. `fortune-free.js`의 `upgradeBanner`를 원래 문구로 복원 (현재 빈 문자열). 자미두수·점성술 배너 블록은 git 이력에서 가져와야 한다.
3. `terms.html` 제3~4조, `privacy.html` 1~4항을 결제 기준으로 되돌리기.
4. 푸터의 `.free-notice` 문구 제거.
5. `pricing.html`의 noindex 해제 + sitemap/robots 복원.

### 법적 문서를 함께 고친 이유
약관에 구독·환불 조항이, 개인정보처리방침에 결제 정보 수집과 Stripe 위탁이 적혀 있었다. 실제로는 그런 기능이 없으므로 **사실과 다른 고지**가 되고, 애드센스 심사에서도 신뢰도에 불리하다. 지금은 "회원가입·결제를 운영하지 않으며 계정 정보를 수집하지 않는다"로 맞췄다.
...
## 2026-07-31 법적 문서 다국어화

### 기존 번역을 재사용하지 않은 이유
`auth.html` 모달에 5개 언어 약관·개인정보가 이미 있었지만, 그건 **유료 구독 기준**으로 쓰인 문서다("월정액 자동 결제", "Stripe 결제 처리"). 지금 한국어 본문은 "결제 기능 없음"이라 그대로 가져다 쓰면 **언어마다 서로 다른 말을 하는 문서**가 된다. 그래서 현재 한국어 본문을 기준으로 새로 번역했다.

### 구조
- `js/legal/{en,ja,zh,es}.js` — `{privacy,terms,contact}` 각각 `{title, desc, main}`. `main`은 `<main>` 안의 HTML 전체.
- `js/legal-i18n.js` — `<body data-legal-page="privacy">` 표시를 보고 해당 문서를 교체한다. 본문·`<title>`·meta description·`<html lang>`·네비 CTA를 함께 바꾼다.
- HTML에는 **한국어 본문을 정적으로 남겨** 크롤러와 애드센스 심사가 읽게 하고, 스크립트가 방문자 언어로 갈아끼운다. 사전 렌더링 작업의 효과를 유지하는 구조.
- 이 페이지들에는 사이트 `i18n.js`를 싣지 않는다(사전이 본문을 덧칠할 수 있음). 대신 자체 `<select>`를 붙였다.

### 내가 만든 결함 하나 — 약관 조문 번호
무료 전환 때 환불 조항(제5조)을 지우면서 번호가 1,2,3,4,6,7…로 **5번이 비었다.** 번역 에이전트 세 곳이 모두 이 점을 지적했다(지시대로 구조를 바꾸지 않고 그대로 옮긴 뒤 보고). 한국어 원문을 1~10으로 다시 매기고 제3조 안의 상호참조도 제9조로 고쳤다.

`scripts/fix-legal-articles.js`는 **문서 순서대로 1..N을 다시 부여**하는 방식이라 몇 번을 돌려도 결과가 같다. 처음에 "6->5, 7->6" 식의 상대 이동으로 짰다가 두 번 실행해 번호가 어긋났던 경험 때문에 순차 재부여로 바꿨다. 상대 이동 방식은 멱등하지 않으니 쓰지 말 것.

## 2026-07-31 도메인 이전

`saju0523.pages.dev` -> `www.trinityofdestiny.com`. 카드 하단 CTA만 바꾸면 되는 줄 알았지만 실제로는 **14개 파일 24곳**이었다.
- 카드 CTA 2곳 (`js/infographic/cards.js` — 일반 카드와 궁합 카드가 각각 별도 마크업)
- canonical 12곳, `sitemap.xml` 9곳, `robots.txt`의 Sitemap 줄
- `auth.html`의 Supabase 이메일 리디렉트 2곳 (**회원가입을 다시 열 때 Supabase 대시보드의 Redirect URL 허용 목록도 새 도메인으로 바꿔야 한다** — 안 바꾸면 인증 메일 링크가 깨진다)

`scripts/swap-domain.py`로 일괄 처리했다. 산출물(`output/`, `rendered_*`)과 `node_modules`는 제외한다. 도메인이 또 바뀌면 파일 상단 두 상수만 고쳐 다시 돌리면 된다.

### 사용자가 별도로 해야 하는 것
1. Cloudflare Pages에 커스텀 도메인 연결
2. 구 도메인에서 새 도메인으로 301 리디렉트 (SEO 자산 승계)
3. Search Console에 새 속성 등록 + sitemap 재제출

## 2026-07-31 애드센스 스니펫

게시자 ID `ca-pub-6708143899799474`. `scripts/add-adsense.py`로 20개 페이지 `<head>`에 넣었다. `remove` 인자를 주면 걷어낸다(마커 주석 기준이라 안전).

`auth-callback.html`은 제외했다. 인증 후 즉시 이동하는 중간 페이지라 광고가 뜰 시간도 없고 의미도 없다.

### 승인 후 확인할 것
- **결과 화면(`/fortune/`)은 렌더링·언어 전환 때마다 본문을 통째로 갈아끼운다.** 자동 광고가 그 안에 자리를 잡으면 재렌더링 시 사라지거나 다시 삽입되며 화면이 밀릴 수 있다. 증상이 보이면 자동 광고 대신 수동 단위를 재렌더링 영향을 받지 않는 위치(섹션 사이, 카드 묶음 아래)에 고정하는 편이 낫다.
- 모바일 전면 광고(vignette)는 끄는 쪽을 권한다. 결과 화면 스크롤이 길어 읽는 도중 튀어나오면 이탈로 이어진다.
...
## 2026-08-08 마운트 전 푸터가 화면 위에 보이던 문제

사용자가 "첫 화면 뜨기 전에 무료라고 한글이 뜬다"고 했고 화면까지 잡아 보내줬다. 원인은 SPA 컨테이너가 비어 있는 동안 **푸터가 화면 맨 위에 그려지는 것**이었다.

| 페이지 | 마운트 전 푸터 위치 | 결과 |
|---|---|---|
| `/` | top 0 | 스플래시 오버레이(z-index 99999)가 덮어 안 보임 |
| `/app/` | top 0 | **그대로 노출** |
| `/fortune/` | top 128 | **그대로 노출** |
| `/goonghap` | top 1068 | 정적 콘텐츠가 있어 접힘선 아래 |

즉 스플래시가 있는 홈만 우연히 가려졌고, 정작 사용자가 많이 가는 계산기·결과 화면이 노출됐다.

### 해결
`site-footer.css`에 이것만 추가했다.

    #root:empty, #fortune-root:empty { display:block; min-height:100vh; }

컨테이너가 비어 있는 동안만 한 화면 높이를 확보해 푸터를 접힘선 아래로 민다. 내용이 채워지면 `:empty`가 풀려 **자동 해제**되므로 JS도, 되돌리는 코드도 필요 없다. 세 컨테이너 모두 안에 공백조차 없어서 `:empty`가 정확히 매칭된다(공백이 있으면 매칭 안 되니 주의).

푸터를 `visibility:hidden`으로 숨겼다가 JS로 켜는 방법도 있었지만, JS가 실패하면 법적 링크가 영영 안 보이므로 쓰지 않았다.
